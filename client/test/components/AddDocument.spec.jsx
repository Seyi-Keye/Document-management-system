...
import { AddDocument, stateToProps, dispatchToProps } from '../../app/components/document/AddDocument';
...
describe('Describe <AddDocument /> Component:', () => {
  ...
  it('should fire handle change', () => {
    ...
    wrapper.find('input').first().simulate('change', { target: { name: 'title', value: 'wathever' } });
  ...
  it('should fire handleSubmit', () => {
    ...
    wrapper = shallow(<AddDocument handleCreateDocument={handleCreateDocumentSpy} />);

    wrapper.find('button').first().simulate('click', { target: { name: 'title' }, preventDefault: () => null });
    expect(browserHistorySpy.calledOnce).to.equal(true);
    expect(handleSubmitSpy.calledOnce).to.equal(true);
    expect(handleCreateDocumentSpy.calledOnce).to.equal(true);
    browserHistorySpy.restore();
    handleSubmitSpy.reset();
  });
  it('should have a state in its props', () => {
    const state = { document: { document: [{ title: 'good', content: 'morning', access: 'public'}] } };
    expect(stateToProps(state).documents).to.deep.equal(state.document);
  });
  it('should dispatch to props', () => {
    const dispatch = () => null;
    expect(typeof dispatchToProps(dispatch).handleCreateDocument).to.equal('function');
    expect(dispatchToProps(dispatch).handleCreateDocument()).to.equal(null);
  });
});