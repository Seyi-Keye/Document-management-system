...
export default function user(state = adminIntialState, action) {
  switch (action.type) {
    ...
    case ActionTypes.UPDATE_USER_SUCCESSFUL: {
      ...
      oldUser.id === action.response.id ? Object.assign({}, oldUser,
    ...
    case ActionTypes.DELETE_USER_SUCCESSFUL: {
      const newUsers = state.users.filter(oldUser =>
      oldUser.id !== parseInt(action.response, 10));
    ...
    case ActionTypes.SEARCH_USER_REQUEST:
      return { ...state, isLoading: true };
    case ActionTypes.SEARCH_USER_SUCCESSFUL:
      return { ...state, users: action.response.user };
